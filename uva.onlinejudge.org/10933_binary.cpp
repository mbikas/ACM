...
void main()
{
	...
	while(scanf("%d",&T)==1 && T!=0)
	{
		...
		qsort(matches,ct,sizeof(match),comp1);
...
void tokenize(char ch[])
{
	...
	ind1 = binary_search(team1,ct);
	ind2 = binary_search(team2,ct);
...
int binary_search(char a[],int m)
{
  ...
  while(beg<=end && ( strcmp(a,matches[mid].name)!=0) )
  {
	 if( strcmp(a,matches[mid].name)<0 )
	  end=end-1;
	 if( strcmp(a,matches[mid].name)>0 )
	  beg=beg+1;
